...
- **Maven Settings**: Builds require `.mvn/settings.xml` (JFrog Artifactory, Sonatype credentials via env vars). CI passes it with `-s .mvn/settings.xml`.
