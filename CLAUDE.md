...
- **Renaming this template is not only a search-and-replace.** `ui/src/assets/app-icon.svg` is a placeholder and binary, so no rename touches it; the About reference screenshot (`ui/test/expected/About/about-loaded.png`) has to be regenerated with `npm --prefix ui run test:update:docker` once the name and icon are the extension's own.
